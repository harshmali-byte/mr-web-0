...
import BlogSEOModel from '../../Models/Blog/BlogSEOModel';
import BlogSEOValidationModel from '../../Models/Blog/BlogSEOValidationModel';
import FormTextField from '../../Common/Inputs/FormTextField';
import { FormHandler } from '../../Common/FormHandler';
import Loader from '../../Common/Loader';

export default function UpsertBlogSeo({ OnDoneUpsertAction, UpsertPressed, EditModel, setIsUpserted }) {
    const [IsLoading, setIsLoading] = useState(false);
    const [Model, setModel] = useState(null);
    const [ValidationErrors, setValidationErrors] = useState([]);
    const [IsValidForm, setIsValidForm] = useState(false);

    const validation = new BlogSEOValidationModel();
    let upsertAbortController = new AbortController();

    useEffect(() => {
        setModel(new BlogSEOModel(EditModel));
        setIsUpserted(true);
    }, [EditModel])
    ...
    function upsertModel(upsertAbortController) {
        ...
        if (Model.Id <= 0) {
            OnDoneUpsertAction(EditModel, { Message: "Please Add Blog Details first", Severity: 'error' }, true);
            return;
        }

        let saveModel = Model;
        if (!saveModel) {
            saveModel = new BlogSEOModel();
        }
        ...
        ApiHandler.ApiService.Post(saveModel, ApiHandler.ApiUrls.BlogAdmin.UpsertSeo, upsertAbortController)
            .then(
                (result) => {
                    ...
                    if (result) {
                        if (result.IsSuccess && result.Data) {
                            ...
                            message = { Message: result.Message || `Successfully ${Model && Model.Id > 0 ? 'updated' : 'added'} SEO for blog post`, Severity: 'success' };
                        }
                        else {
                            if (result.Data && result.Data.Errors && result.Data.Errors.length > 0) {
                                ...
                                message = { Message: result.Message || `Fail to ${Model && Model.Id > 0 ? 'update' : 'add'} SEO for blog post`, Severity: 'error' };
                            }
                        }
                    }
                    else
