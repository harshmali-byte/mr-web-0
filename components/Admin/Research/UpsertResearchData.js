...
import { Card, CardContent, FormControl, Grid, Button, Divider, Paper, IconButton } from '@mui/material';
...
import ResearchInfoValidationModel from '../../Models/Research/ResearchInfoValidationModel';
import FormTextField from '../../Common/Inputs/FormTextField';
import { FormHandler } from '../../Common/FormHandler';
import Image from 'next/image';
import DeleteForeverIcon from '@mui/icons-material/DeleteForever';
import UpsertResearchDataRules from './UpsertResearchDataRules';

export default function UpsertResearchData({ OnDoneUpsertAction, UpsertPressed, EditModel, infoName, infoDisplayName, setIsUpserted, setToast, uploadImageDisable }) {
    ...
    const validation = new ResearchInfoValidationModel();
    ...
    function fetchInfo() {
        if (!EditModel || EditModel.Id === 0) {
            setModel(new ResearchInfoModel(EditModel));
            return;
        }
        ...
        ApiHandler.ApiService.Post(model, ApiHandler.ApiUrls.ResearchInfo.GetInfoById, fetchInfoAbortController)
            .then(
                (result) => {
                    ...
                    let model = new ResearchInfoModel(EditModel)
                    model[infoName] = result.Data[infoName];
                    setModel(model);
                },
                (error) => {
                    console.log(error);
                    setToast({ Message: "Error occurred to find research. Please contact IT team", Severity: 'error' });
                }
            )
    }
    ...
    function fetchImagesPaths() {
        ...
        ApiHandler.ApiService.Post(model, ApiHandler.ApiUrls.ResearchAdmin.GetImagesPaths, imagesAbortController)
            .then(
                (result) => {
    ...
    function fetchImage(path) {
        ...
        ApiHandler.ApiService.PostDownloadCancellable(model, ApiHandler.ApiUrls.ResearchAdmin.GetImage, imageAbortController)
            .then(
                (result) => {
    ...
    function upsertModel(upsertAbortController) {
        ...
        let saveModel = Model;
        if (!saveModel) {
            saveModel = new ResearchInfoModel();
        }
        ...
        ApiHandler.ApiService.Post(saveModel, ApiHandler.ApiUrls.ResearchAdmin[`Upsert${infoName}`], upsertAbortController)
            .then(
                (result) => {
                    ...
                    if (result) {
                        if (result.IsSuccess && result.Data) {
                            ...
                            message = { Message: result.Message || `Successfully ${EditModel && EditModel.Id > 0 ? 'updated' : 'added'} ${infoDisplayName} for reasearch post`, Severity: 'success' };
                        }
                        else {
                            message = { Message: result.Message || `Fail to ${EditModel && EditModel.Id > 0 ? 'update' : 'add'} ${infoDisplayName} for reasearch post`, Severity: 'error' };
                        }
                    }
                    else {
    ...
    function uploadImages() {
        ...
        ApiHandler.ApiService.Post(formData, ApiHandler.ApiUrls.ResearchAdmin.UploadImages, null, true)
            .then(
                (result) => {
    ...
    function onDeleteImage(img) {
        ...
        ApiHandler.ApiService.Post(model, ApiHandler.ApiUrls.ResearchAdmin.DeleteImage)
            .then(
                (result) => {
                    try {
                        if (result && result.IsSuccess) {
    ...
    function showImages() {
        ...
        return (
            <Grid item container xs={12} spacing={1} sx={{ display: 'flex', justifyContent: 'flex-start' }}>
                {
                    ...
                    <Grid container spacing={1} rowSpacing={3}>
                        ...
                        <Grid item xs={12}>
                            <FormControl fullWidth >
                                <FormTextField fieldError={setError(infoName)} label={infoDisplayName} value={Model[infoName]} onChange={setFormTextValues(infoName)} multiline={true} />
                            </FormControl>
                        </Grid>
                    </Grid>
                </form>
                <UpsertResearchDataRules hideImageRule={uploadImageDisable} />
            </CardContent>
        </Card>
    )
}