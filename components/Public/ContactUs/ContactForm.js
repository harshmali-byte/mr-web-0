...
import ContactRequestQueryModel from '../../Models/Request/ContactRequestQueryModel';
import ContactRequestValidationModel from '../../Models/Request/ContactRequestValidationModel';
import CaptchaModel from '../../Models/Captcha/CaptchaModel';
import ContactRequestModel from '../../Models/Request/ContactRequestModel';
import { SessionStorage } from '../../Common/SessionStorage';
import CaptchaValidationModel from '../../Models/Captcha/CaptchaValidationModel';

export default function ContactForm() {
    const requestType = 'ContactUs';
    ...
    const [Model, setModel] = useState(new ContactRequestQueryModel());
    ...
    const validation = new ContactRequestValidationModel();
    ...
    function SaveRequest() {
        ...
        let saveModel = Model;
        if (!saveModel) {
            saveModel = new ContactRequestQueryModel();
        }

        if (!saveModel.Request) {
            saveModel.Request = new ContactRequestModel();
        }
    ...
    const setRequestJobTitleValues = (prop) => (event, jobTitle) => {
    ...
    function buildJobTitleComponent() {
        ...
        return (
            <Autocomplete
                ...
                autoHighlight
                value={JobTitle}
                onChange={setRequestJobTitleValues('JobTitleId')}
                ...
                renderInput={(params) => (
                    <TextField
                        ...
                        label="Job Title *"
                        variant="standard"
                        error={setError('JobTitleId').IsError} helperText={setError('JobTitleId').ErrorMessage}
                        inputProps={{
                            ...params.inputProps,
                            autoComplete: 'new-password', // disable autocomplete and autofill
                        }}
    ...
    return (
        <Paper id="request-form" sx={{}} elevation={0}>
            <Box sx={{ pb: 1 }} >
                <Typography id="Login-title" variant="h6" component="h2" color="primary" sx={{ fontSize: 18, fontWeight: '600' }}>
                ...
                <Grid container spacing={3}>
                    <Grid item xs={12} md={6}>
                        <FormControl fullWidth >
                            <TextField variant="standard" label="Your Name *" value={Model.Request.FullName} onKeyPress={KeyPress} onChange={setRequestTextValues('FullName')}
                                error={setError('FullName').IsError} helperText={setError('FullName').ErrorMessage} />
                        </FormControl>
                    </Grid>
                    <Grid item xs={12} md={6}>
                        <FormControl fullWidth>
                            <TextField variant="standard" type='text' label="Company Name *" value={Model.Request.CompanyName} onKeyPress={KeyPress} onChange={setRequestTextValues('CompanyName')}
